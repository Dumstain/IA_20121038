...
# --- 3. Preparar Datos y Entrenar MODELO HORIZONTAL (KNN) ---
print("\n--- Entrenando K-Vecinos Cercanos para Movimiento Horizontal ---")
...
# Elegir k basado en el tamaño del dataset
k_horizontal = min(7, int(len(X_h_train) ** 0.5))  # Regla heurística para k
if k_horizontal % 2 == 0:  # Hacer k impar para evitar empates
    k_horizontal += 1

knn_horizontal = KNeighborsClassifier(
    n_neighbors=k_horizontal,
    weights='distance',  # Dar más peso a los vecinos más cercanos
    algorithm='auto',
    metric='minkowski'
)

knn_horizontal.fit(X_h_train_scaled, y_h_train)

# Evaluar modelo horizontal
y_h_pred = knn_horizontal.predict(X_h_test_scaled)
accuracy_h = accuracy_score(y_h_test, y_h_pred)
print(f"Precisión del Modelo Horizontal (prueba): {accuracy_h:.4f}")
print(f"K utilizado: {k_horizontal}")
...
path_modelo_h = os.path.join(CARPETA_MODELOS, 'knn_horizontal_model.joblib')
path_scaler_h = os.path.join(CARPETA_MODELOS, 'knn_horizontal_scaler.joblib')
joblib.dump(knn_horizontal, path_modelo_h)
...
# --- 4. Preparar Datos y Entrenar MODELO DE SALTO (KNN) ---
print("\n--- Entrenando K-Vecinos Cercanos para Salto ---")
...
# Crear y entrenar KNN para salto
k_salto = min(5, int(len(X_s_train) ** 0.5))  # K más conservador para salto
if k_salto % 2 == 0:
    k_salto += 1
...
path_modelo_s = os.path.join(CARPETA_MODELOS, 'knn_jump_model.joblib')
path_scaler_s = os.path.join(CARPETA_MODELOS, 'knn_jump_scaler.joblib')
joblib.dump(knn_salto, path_modelo_s)
...
print("\n¡Entrenamiento de K-Vecinos Cercanos completado!")
